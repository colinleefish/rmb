...
func TestParseBuildScenesResponse(t *testing.T) {
	valid := map[string]struct{}{
		"mem9://sessions/x/atoms/a1": {},
		"mem9://sessions/x/atoms/a2": {},
	}
	raw := `{"scenes":[{"display_name":"Setup","abstract":"Hook setup.","body":"## Setup\nConfigured hooks.","atom_uris":["mem9://sessions/x/atoms/a1","mem9://sessions/x/atoms/a2"]}]}`
...
func TestParseBuildScenesResponse_dropsUnknownURIsKeepsValid(t *testing.T) {
	...
	raw := `{"scenes":[
		{"display_name":"Keep","abstract":"a","body":"b","atom_uris":["mem9://sessions/x/atoms/a1","mem9://hallucinated"]},
		{"display_name":"Drop","abstract":"a","body":"b","atom_uris":["mem9://hallucinated"]}
	]}`
	scenes, err := parseBuildScenesResponse(raw, valid)
	if err != nil {
	...
	if scenes[0].DisplayName != "Keep" || len(scenes[0].SourceAtomURIs) != 1 ||
		scenes[0].SourceAtomURIs[0] != "mem9://sessions/x/atoms/a1" {
...
func TestParseBuildScenesResponse_allUnknownErrors(t *testing.T) {
	valid := map[string]struct{}{"mem9://sessions/x/atoms/a1": {}}
	raw := `{"scenes":[{"display_name":"X","abstract":"a","body":"b","atom_uris":["mem9://bad"]}]}`
...
func TestParseBuildScenesResponse_fence(t *testing.T) {
	valid := map[string]struct{}{"mem9://sessions/x/atoms/a1": {}}
	raw := "```json\n{\"scenes\":[{\"display_name\":\"X\",\"abstract\":\"a\",\"body\":\"b\",\"atom_uris\":[\"mem9://sessions/x/atoms/a1\"]}]}\n```"
	scenes, err := parseBuildScenesResponse(raw, valid)
	if err != nil {
		t.Fatal(err)
	}
