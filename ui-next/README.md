# RMB Observer — `ui-next`

A modern rebuild of the RMB Observer UI using **Next.js (App Router) + TypeScript + Tailwind + shadcn/ui**. It consumes the existing rmb Go API (`/api/v1/browse/*`).
...
- **Dev** (`pnpm dev`): `src/app/api/v1/[...path]/route.ts` is a same-origin proxy that forwards `/api/v1/*` to `RMB_API_URL`, injecting optional Basic-auth credentials server-side (so they never reach the browser) and avoiding CORS.
...
```
RMB_API_URL=http://localhost:8080   # upstream API base (serves /api/v1/*)
RMB_API_USER=                       # optional Basic-auth user
RMB_API_PASS=                       # optional Basic-auth password
```

Point `RMB_API_URL` at a local Go binary (`http://localhost:8080`) or the
remote server (`https://rmb.colinleefish.com`, which requires the Basic-auth
