...
rmb currently distills and never forgets. Borrow the Ebbinghaus forgetting
...
- **v1** — record hits + make them visible (`rmb memory stats`). No ranking
  change.
...
`rmb eval` calls the low-level `recall.FTSMemories` / `recall.VectorMemories`
