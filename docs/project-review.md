# Project review: mypast vs TDAI and OpenViking

> Status: review note (2026-06). An end-to-end assessment of mypast and a
...
> local-only), so the TDAI/OpenViking columns draw on mypast's own design-doc
> characterization (`design-l0-l4.md` §3, §13) plus general knowledge, not their
> source. Anything inferred is marked as such. The mypast assessment is grounded
> in the current code.
...
| Dimension | mypast | TDAI (Tencent) | OpenViking |
|-----------|--------|----------------|------------|
...
| Extraction | 4-category atoms, in-call scene segmentation (from TDAI) | L1 atom extraction (the model mypast copied) | 8-category user model |
| Facets | 2 (abstract / body) — dropped the middle "overview" | — | 3 (abstract / overview / detail) |
| Addressing | `mypast://` URI scheme (from OpenViking) | — | `viking://` URIs + directory tree |
| Retrieval | vector + FTS + RRF fusion (hierarchical score-prop deferred) | persona / timer driven | hierarchical descent with score propagation (mypast's design goal) |
...
## Where mypast is genuinely stronger
...
1. **Tool-agnostic, zero-SDK capture.** Anything that can fire a hook and POST
   ...
   framing). This is mypast's clearest differentiator.
...
3. **Consolidation discipline.** mypast is explicitly built around the
...
## Where mypast is behind / weaker

1. **Retrieval is the least-finished tier.** mypast borrowed OpenViking's idea
...
4. **No usage / forgetting signal yet.** TDAI's persona model implicitly
   re-weights; mypast keeps everything forever (the Ebbinghaus plan addresses
...
mypast is the most operationally disciplined and capture-flexible of the three,
